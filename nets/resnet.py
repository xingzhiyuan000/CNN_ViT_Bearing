...
if __name__ == "__main__":
    ...
    model = ResNet(Bottleneck, [3, 4, 6, 3]).to(device)  # 实例化网络模型
    model = model.to(device)  # 将模型转移到cuda上
    input = torch.ones((1,1,10, 10))  # 生成一个batchsize为64的，通道数为1，宽度为2048的信号
    input = input.to(device)  # 将数据转移到cuda上
    output = model(input)  # 将输入喂入网络中进行处理

    summary(model, input_size=(1,10,10))  # 输入一个通道为1的宽度为2048，并展示出网络模型结构和参数
