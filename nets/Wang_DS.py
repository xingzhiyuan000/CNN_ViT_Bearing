...
# 搭建神经网络
class Wang_DS(nn.Module):
    def __init__(self):
        super(Wang_DS, self).__init__()
        # 1X10X10------32X10X10
...
if __name__ == '__main__':
    ...
    wang_DS=Wang_DS() #实例化网络模型
    wang_DS=wang_DS.to(device) #将模型转移到cuda上
    input=torch.ones((64,1,10,10)) #生成一个batchsize为64的，1个通道的10X10 tensor矩阵-可以用来检查网络输出大小
    input=input.to(device) #将数据转移到cuda上
    output=wang_DS(input) #将输入喂入网络中进行处理
    print(output.shape)
    summary(wang_DS,input_size=(1,10,10)) #输入一个通道为1的10X10数据，并展示出网络模型结构和参数
