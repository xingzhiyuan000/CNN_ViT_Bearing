...
# 搭建神经网络
class Wang_DS_RGB_Dropout(nn.Module):
    def __init__(self):
        super(Wang_DS_RGB_Dropout, self).__init__()
        # 1X10X10------32X10X10
...
if __name__ == '__main__':
    ...
    Wang_DS_RGB_Dropout=Wang_DS_RGB_Dropout() #实例化网络模型
    Wang_DS_RGB_Dropout=Wang_DS_RGB_Dropout.to(device) #将模型转移到cuda上
    input=torch.ones((32,3,10,10)) #生成一个batchsize为64的，1个通道的10X10 tensor矩阵-可以用来检查网络输出大小
    input=input.to(device) #将数据转移到cuda上
    output=Wang_DS_RGB_Dropout(input) #将输入喂入网络中进行处理
    print(output.shape)
    summary(Wang_DS_RGB_Dropout,input_size=(3,10,10)) #输入一个通道为3的10X10数据，并展示出网络模型结构和参数
