...
class DropPath(nn.Module):
    """
    Drop paths (Stochastic Depth) per sample  (when applied in main path of residual blocks).123
    """
...
class Wang_Normal_ViT_RGB(nn.Module):
    def __init__(self):
        ...
        self.conv1 =nn.Conv2d(in_channels=1,out_channels=64,kernel_size=3,stride=1,padding=1,dilation=1)
    ...
    def forward(self, x):
        ...
        return x
# 搭建神经网络-输出为10X10
class Wang_100(nn.Module):
    def __init__(self):
        super(Wang_100, self).__init__()
        # 3X10X10------64X10X10
        # self.conv1 = nn.Conv2d(in_channels=1, out_channels=32, kernel_size=3, stride=1, padding=1) #普通3X3卷积
        self.conv1 =nn.Conv2d(in_channels=1,out_channels=512,kernel_size=1,stride=1,padding=0,dilation=1)
        #self.conv1 =depthwise_separable_conv(3, 64) #深度可分离3X3卷积
...
if __name__ == '__main__':
    ...
    print(output.shape)
    summary(model,input_size=(1,10,10)) #输入一个通道为3的10X10数据，并展示出网络模型结构和参数
